...
# 定义裁剪区域
start_row, start_col = int(height * 0.25), int(width * 0.2)
...
if len(rectangles)>=2:
    ...
    print(largest_rectangle.shape)
    # 在图像上绘制该矩形
    cv2.drawContours(img, [largest_rectangle], 0, (0, 0, 255), 6)
    cv2.drawContours(img, [second_largest_rectangle], 0, (0, 255, 0), 6)

    # result = cv2.pointPolygonTest(largest_rectangle, tuple(point), False)
    # result1 = cv2.pointPolygonTest(second_largest_rectangle, tuple(point), False)
    ...
    #     print("res:",3)
cv2.imshow('Image with rectangle contours', edges)
cv2.imwrite('result.png',img,[int(cv2.IMWRITE_JPEG_QUALITY),70])
cv2.waitKey(0)
